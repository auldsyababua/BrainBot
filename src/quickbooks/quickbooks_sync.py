...
class QuickBooksSync:
    ...
    async def init_auth(self):
        ...
        company = supabase.table("companies").select("*").eq("id", self.company_id).execute()
        if not company.data:
            raise Exception(f"Company {self.company_id} not found")
        
        self.realm_id = company.data[0].get("quickbooks_realm_id")
        if not self.realm_id:
            raise Exception("Company not connected to QuickBooks. Run connect_quickbooks() first.")
        
        # Get valid auth token
        auth = supabase.table("quickbooks_auth").select("*").eq("company_id", self.company_id).eq("is_active", True).execute()
        if not auth.data:
            raise Exception("No active QuickBooks authentication found")
        
        self.access_token = auth.data[0]["access_token"]
    
    async def query_api(self, endpoint: str, params: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    async def pull_accounts(self):
        ...
        # Query all accounts
        response = await self.query_api("query", {
    ...
    async def sync_account_to_supabase(self, account: Dict[str, Any]):
        ...
        # Check if exists
        existing = supabase.table("chart_of_accounts").select("id").eq("company_id", self.company_id).eq("quickbooks_account_id", account["Id"]).execute()
    ...
    async def pull_customers(self):
        ...
        for customer in track(customers, description="Syncing customers"):
            ...
            # Check if exists
            existing = supabase.table("accounting_customers").select("id").eq("company_id", self.company_id).eq("quickbooks_customer_id", customer["Id"]).execute()
    ...
    async def sync_journal_entry_to_supabase(self, entry: Dict[str, Any]):
        ...
        # Check if exists
        existing = supabase.table("journal_entries").select("id").eq("company_id", self.company_id).eq("quickbooks_journal_entry_id", entry["Id"]).execute()
        ...
        if existing.data:
            ...
            supabase.table("journal_entry_lines").delete().eq("journal_entry_id", je_id).execute()
        else:
        ...
        for idx, line in enumerate(entry.get("Line", [])):
            if line.get("JournalEntryLineDetail"):
                ...
                if account_ref:
                    ...
                    account = supabase.table("chart_of_accounts").select("id").eq("company_id", self.company_id).eq("quickbooks_account_id", account_ref["value"]).execute()
                    ...
                    if account.data:
                        ...
                        supabase.table("journal_entry_lines").insert(line_data).execute()
    ...
    async def push_customer(self, customer_id: str):
        ...
        customer = supabase.table("accounting_customers").select("*").eq("id", customer_id).execute()
    ...
    async def push_journal_entry(self, journal_entry_id: str):
        ...
        je = supabase.table("journal_entries").select("*, journal_entry_lines(*)").eq("id", journal_entry_id).execute()
        if not je.data:
        ...
        for line in entry.get("journal_entry_lines", []):
            ...
            account = supabase.table("chart_of_accounts").select("quickbooks_account_id").eq("id", line["account_id"]).execute()
            
            if account.data and account.data[0].get("quickbooks_account_id"):
                qb_line = {
                    "Amount": str(line["debit"] if line["debit"] > 0 else line["credit"]),
                    "Description": line.get("description", ""),
    ...
    async def list_accounts(self) -> List[Dict[str, Any]]:
        ...
        response = await self.query_api("query", {
            "query": "SELECT * FROM Account ORDERBY Name"
        })
    ...
    async def show_sync_status(self):
        ...
        qb_accounts = await self.query_api("query", {"query": "SELECT COUNT(*) FROM Account"})
        qb_customers = await self.query_api("query", {"query": "SELECT COUNT(*) FROM Customer"})
        qb_vendors = await self.query_api("query", {"query": "SELECT COUNT(*) FROM Vendor"})
        
        # Get counts from Supabase
        sb_accounts = supabase.table("chart_of_accounts").select("id", count="exact").eq("company_id", self.company_id).execute()
        sb_customers = supabase.table("accounting_customers").select("id", count="exact").eq("company_id", self.company_id).execute()
...
async def connect_quickbooks(company_id: str):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(f"http://localhost:8000/auth/quickbooks?company_id={company_id}")
        data = response.json()
...
async def show_quickbooks_data(company_id: str):
    ...
    async with QuickBooksSync(company_id) as sync:
        ...
        response = await sync.query_api("query", {
        ...
        if entries:
            ...
            for entry in entries:
                total = sum(float(line.get("Amount", 0)) for line in entry.get("Line", []) if line.get("JournalEntryLineDetail", {}).get("PostingType") == "Debit")
                table.add_row(
                    entry["TxnDate"],
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        console.print("[red]Usage: python quickbooks_sync.py <command> <company_id> [options][/red]")
        console.print("\nCommands:")
        console.print("  connect <company_id>              - Connect to QuickBooks")
        console.print("  pull <company_id>                 - Pull all data from QuickBooks")
