...
class KeywordRouter:
    ...
    def _check_cache(self, message_hash: int) -> Optional[RouteResult]:
        ...
        if message_hash in self._recent_routes:
            # Check if cache is still valid
            if time.time() - self._cache_timestamps.get(message_hash, 0) < self._cache_ttl:
                return self._recent_routes[message_hash]

        return None

    def _preprocess_message(self, message: str) -> Tuple[str, Dict[str, Any]]:
        """Preprocess message for pattern matching."""
        ...
        # Combine prefilled data with confidences for metadata
        metadata = {
    ...
    def _match_patterns(
        self, processed_message: str, metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """Match message against all configured patterns."""
        matches = []
        message_lower = metadata["message_lower"]
        prefilled_data = metadata["prefilled_data"]
        confidences = metadata["confidences"]
        cleaned_message = metadata["cleaned_message"]
        ...
        # Check for hidden commands (100% confidence)
        ...
        telegram_boost = 0.0
        forced_entity_type = prefilled_data.get("entity_type")
        ...
        # Handle Telegram command without operation match
        if forced_entity_type and not matches:
            matches.append(
                {
    ...
    def _calculate_final_confidence(
        self, matches: List[Dict[str, Any]], metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Calculate final confidence score from pattern matches."""
        if not matches:
            return None
        ...
        for match_info in matches:
            # Skip if this is a hidden command (already has 100% confidence)
            if match_info.get("is_hidden"):
                return match_info

            confidence = match_info["confidence"]
            entity_type = match_info["entity_type"]
            operation = match_info["operation"]
            target_users = match_info.get("target_users", [])

            # Apply confidence adjustments

            # Reduce confidence for uncertain language
            uncertainty_words = ["maybe", "possibly", "perhaps", "could", "might", "what about"]
            ...
            if any(
                word in message_lower for word in ["what", "when", "who", "where", "why", "how"]
            ):
            ...
            match_info["final_confidence"] = confidence

            # Check if this is the best match
            match_length = match_info.get("match_length", 0)
            should_update = confidence > best_confidence or (
                confidence == best_confidence
                and match_length > getattr(best_match, "match_length", 0)
            )
    ...
    def route(self, message: str) -> RouteResult:
        ...
        # Process and match
        cleaned_message, metadata = self._preprocess_message(message)
        prefilled_data = metadata["prefilled_data"]
        confidences = metadata["confidences"]
        message_lower = metadata["message_lower"]
        ...
        if prefilled_data.get("entity_type") and prefilled_data.get("operation"):
            ...
            self._recent_routes[cache_key] = result
            self._cache_timestamps[cache_key] = time.time()
            self._enforce_cache_size_limit()
            return result

        # Match patterns and calculate confidence
        matches = self._match_patterns(cleaned_message, metadata)
        best_match_info = self._calculate_final_confidence(matches, metadata)

        # Build result from best match
        if best_match_info:
            final_confidence = best_match_info.get(
                "final_confidence", best_match_info.get("confidence", 0.0)
            )
            target_users = best_match_info.get("target_users", [])

            result = RouteResult(
                entity_type=best_match_info["entity_type"],
                operation=best_match_info["operation"],
                function_name=best_match_info.get("function_name"),
                confidence=final_confidence,
                extracted_data=best_match_info.get("extracted_data", {}),
                use_direct_execution=(
                    final_confidence >= 0.8 or best_match_info.get("is_hidden", False)
                ),
                target_users=target_users,
                entity_confidence=best_match_info.get("confidences", {}).get(
                    "entity_confidence", final_confidence
                ),
                operation_confidence=final_confidence,
                assignee_confidence=best_match_info.get("confidences", {}).get(
                    "assignee_confidence"
                ),
            )
        else:
            result = RouteResult(None, None, None, 0.0)
