...
# Map account names to account IDs - use the mapping we already created
account_mapping = {
    # Assets
...
print(f"Found {len(account_mapping)} accounts in chart of accounts")
...
with open("insert_journal_entries.sql", "w") as f:
    ...
    for (date, trans_group, trans_type), group_df in transaction_groups:
        ...
        # Calculate totals
        total_debit = group_df["Debit"].sum()
        total_credit = group_df["Credit"].sum()

        # Get description from first non-empty memo
        description = None
        for memo in group_df["Memo"]:
            if pd.notna(memo) and str(memo).strip():
                description = escape_sql(str(memo))
                break
        if not description:
            description = escape_sql(f"{trans_type} - {date.strftime('%Y-%m-%d')}")
...
\n"""
        ...
        for idx, line in group_df.iterrows():
            ...
            if not account_id:
                print(f"Warning: Could not map account '{account_name}'")
                continue
            ...
            f.write(
                f"""INSERT INTO public.journal_lines (
    line_id_display, entry_id, account_id, 
    description, amount, is_debit
) VALUES (
    '{line_id}', 
    (SELECT id FROM journal_entries WHERE entry_id_display = '{entry_id}'),
    (SELECT id FROM chart_of_accounts WHERE account_id_display = '{account_id}'),
    '{memo}',
    {amount}, {str(is_debit).lower()}
) ON CONFLICT (line_id_display) DO NOTHING;
\n"""
            )

print(f"\nGenerated SQL for {entry_count} journal entries with {line_count} lines")
print("Output saved to: insert_journal_entries.sql")
