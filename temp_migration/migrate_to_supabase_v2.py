...
trial_balance_df = pd.read_excel(
    "transactions/Trial_balance.xlsx", sheet_name="Trial Balance", skiprows=3
)
...
trial_balance_df = trial_balance_df[
    ~trial_balance_df["account_full"].str.contains("Total", na=False)
]
...
with open("insert_accounts.sql", "w") as f:
    ...
    '{escape_sql(account_data['account_name'])}', 
...
with open("insert_partners.sql", "w") as f:
    ...
    '{partner_id_display}', '{escape_sql(customer_name)}', '{partner_type}',
    {sql_value(row['full_name'])},
    {sql_value(row['email'])},
    true
...
with open("insert_accounting_customers.sql", "w") as f:
    ...
    for _, row in customers_df.iterrows():
        customer_name = row["customer_name"]
        if any(
            customer.lower() in customer_name.lower()
            for customer in CUSTOMERS_CONTRACTORS
        ):
            customer_count += 1
    ...
    '{customer_id_display}', '{COMPANY_ID}', '{escape_sql(customer_name)}',
    {sql_value(row['full_name'])},
    {sql_value(row['email'])},
    {sql_value(row['billing_address'])},
    {sql_value(row['shipping_address'])},
    true
);
"""
            f.write(sql)
...
# Accounting Employees SQL with personnel matching
employees_df = pd.read_excel(
    "transactions/Employees.xlsx", sheet_name="Employee Contact List", skiprows=3
)
...
with open("insert_accounting_employees.sql", "w") as f:
    ...
    f.write("-- TODO: Match these with existing personnel records\n\n")

    for idx, row in employees_df.iterrows():
        employee_id_display = f"EMP-{str(idx+1).zfill(3)}"
        employee_name = row["employee_name"]

        # Add comment about potential personnel match
        f.write(f"-- Employee: {employee_name}\n")
        if "Colin" in employee_name:
            f.write(
    ...
    '{employee_id_display}', '{COMPANY_ID}', '{escape_sql(employee_name)}',
    {sql_value(row['email'])},
    {sql_value(row['phone'])},
    {sql_value(row['address'])},
    true
);
"""
        f.write(sql + "\n")
