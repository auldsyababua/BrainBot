...
with open("insert_journal_entries_final.sql", "w") as f:
...
\n"""
        ...
        # Insert journal lines with proper line numbers
        line_number = 0
        for idx, line in group_df.iterrows():
            ...
            line_number += 1
            debit = line["Debit"] if line["Debit"] > 0 else 0
            credit = line["Credit"] if line["Credit"] > 0 else 0
            description = (
                escape_sql(str(line["Memo"])) if pd.notna(line["Memo"]) else ""
            )
...
print(f"\nGenerated SQL for {entry_count} journal entries")
print(f"\nUnmapped accounts ({len(unmapped_accounts)}):")
for acc in sorted(unmapped_accounts):
    print(f"  - {acc}")
print("\nOutput saved to: insert_journal_entries_final.sql")
